...
# Very basic shell script to prompt a user for a directory of disk images, run the disktype utility against that directory, sort the images depending on file's output, and report how many images were sorted into which category.
...
for file in $STARTDIR/*; do
    if [ -f "$file" ]; then
        FILEOUT=$(disktype $file)
        if [[ $FILEOUT == *"FAT"* ]]; then
            mv $file $STARTDIR/pc/
            #echo Moved $file to PC directory
        elif [[ $FILEOUT == *"HFS"* ]]; then
